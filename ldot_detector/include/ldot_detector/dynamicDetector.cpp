...
void dynamicDetector::runDetection() {
  ...
  if (this->staticFilterEnabled_) {
    for (const auto &track : this->boxHist_) {
      if (!track.empty() && (track[0].is_dynamic || track[0].is_dynamic_candidate)) {
