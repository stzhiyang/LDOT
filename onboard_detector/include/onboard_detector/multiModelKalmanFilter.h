 ...
 * @return 卡尔曼滤波器的智能指针
 */
inline std::shared_ptr<KalmanFilterBase> createKalmanFilter(
    bool is_human, bool is_che, bool is_uav, bool is_else) {
    
    if (is_human) {
        // 人 -> CA-KF (2D)
        auto model = std::make_shared<CA_Model>(false);  // 2D
        return std::make_shared<LinearKalmanFilter>(model);
    } 
    else if (is_uav) {
        // 无人机 -> CA-KF (3D)
        auto model = std::make_shared<CA_Model>(true);   // 3D
        return std::make_shared<LinearKalmanFilter>(model);
    } 
    else if (is_che) {
        // 车 -> CTRA-EKF
        auto model = std::make_shared<CTRA_Model>();
        return std::make_shared<ExtendedKalmanFilter>(model);
    } 
    else {
        // 其他 -> CV-KF (3D)
        auto model = std::make_shared<CV_Model>();
        return std::make_shared<LinearKalmanFilter>(model);
    }
}
