/*
    ...
    Includes: CA (Constant Acceleration), CV (Constant Velocity), CTRA (Constant Turn Rate and Acceleration)
*/
...
public:
     ...
     */
    explicit CA_Model(bool use_3d = false);
    ...
    void normalizeYawInResidual(Eigen::VectorXd& residual) override {} // CA模型无yaw
...
public:
    ...
    void normalizeYawInResidual(Eigen::VectorXd& residual) override {} // CV模型无yaw
};

/**
 * @brief CTRA模型 - 恒定转弯率和加速度模型 (用于车辆)
 * 
 * 状态向量: [x, y, v, a, yaw, yaw_rate]^T (6维)
 * - x, y: 位置
 * - v: 行驶速度 (标量)
 * - a: 加速度
 * - yaw: 偏航角
